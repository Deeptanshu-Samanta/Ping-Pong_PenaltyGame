...
def restart_game(label2=None, button_replay=None):
    ...
    # Reset canvas (black background again)
    canvas.delete("all")
...
def ball_movement():
    ...
    while True:
        ...
        if coordinates[0] < 0 or coordinates[0] >= w - 50:
            xVelocity = -xVelocity
        if coordinates[1] < 0 and 250 < coordinates[0] < 450 - 35:
            score += 1
            score_card(score)
            reset_positions()
            time.sleep(1)
        ...
        if coordinates[1] < 0:
            yVelocity = -yVelocity
        if ((coord[0] < coordinates[0] < coord[2] or coord[0] < coordinates[2] < coord[2]) 
            and coord[1] - 30 < coordinates[1] < coord[3] - 30):
            yVelocity = -yVelocity
...
outs = score = 0
score_label = canvas.create_text(350, 200, text="Score: 0", font=("Times New Roman", 30), fill='brown')
